...
export default function GhettoSlotMachine({ onBackPress }: GhettoSlotMachineProps) {
  ...
  const calculateWinnings = (results: number[]) => {
    ...
    // Check for guns (prioritize higher gun counts first)
  ...
  const handleGangChoice = (gangName: string) => {
    ...
    // 40% chance to win the gang war, increasing slightly with heat level
    const winChance = 0.4 + (heatLevel * 0.05); // Chance increases with heat level
    ...
    if (win) {
      ...
      // Small heat loss on gang war loss
      setHeatLevel(prev => Math.max(prev - 1, 0));
    }
  ...
  const handleRobberyTap = () => {
    ...
    // Fixed amount: 50 credits per tap, scale by heat level (50 × heat multiplier)
    const baseAmount = 50;
    const newRawAmount = baseAmount * newTapCount;
    
    // Apply heat multiplier
    const multiplier = HEAT_LEVELS[heatLevel].multiplier;
    const newMultipliedAmount = Math.floor(newRawAmount * multiplier);
    ...
    console.log(`Tap ${newTapCount}: ${newRawAmount} × ${multiplier} = ${newMultipliedAmount}`);
  ...
  const handlePoliceEncounter = (count: number) => {
    ...
    switch (count) {
      ...
      case 3: // Shot by police - get HUGE settlement money!
        ...
        // Increase heat level on police settlement jackpot
        setHeatLevel(prev => Math.min(prev + 2, HEAT_LEVELS.length - 1));
        setLastWinTime(Date.now());
        break;
    }
  ...
  const handleMultipleFeatures = (features: string[], bonusWin: number) => {
    ...
    // Get count of actual police symbols from the results
    const policeCount = results.filter(r => SYMBOLS[r].name === 'police').length;
    
    // Process all positive features first
    if (features.includes('cash')) {
      const cashSymbols = results.filter(r => SYMBOLS[r].name === 'cash').length;
      combinedMessage += `💵 Cash bonus (${cashSymbols}x)! `;
    }
    ...
    if (features.includes('gangWar') && features.includes('police')) {
      ...
      // Boost reward for this rare combo (influenced by police count)
      const gangWarPoliceBonus = bet * 100 * HEAT_LEVELS[heatLevel].multiplier * (policeCount + 1);
      ...
      if (features.includes('police')) {
        // Use the actual count of police symbols for outcomes
        if (policeCount === 3) {
          // Police settlement
    ...
    // Special handling for overdose - always triggers after other features are processed
    if (features.includes('overdose')) {
      // Overdose (negative)
      const overdoseCost = Math.floor(bet * 5); // Reduced for combined feature
      
      // If we also have Gang War with Police, reduce overdose effect (player is somewhat protected)
      if (features.includes('gangWar') && features.includes('police')) {
        // Reduce the overdose penalty by 50% in this rare case
        totalWin -= Math.floor(overdoseCost / 2);
        combinedMessage += "💊💊💊 Partial Overdose (Gang protection)! ";
      } else {
        totalWin -= overdoseCost;
        combinedMessage += "💊💊💊 Overdose! ";
      }
      
      // Still lose heat for overdose, but not completely if we have gang protection
      if (features.includes('gangWar') || features.includes('robbery')) {
        // Only drop heat by 2 levels if gang-related features are active
        setHeatLevel(prev => Math.max(prev - 2, 0));
      } else {
        // Complete heat loss otherwise
        setHeatLevel(0);
      }
    }
